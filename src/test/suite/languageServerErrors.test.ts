...
const FILE_MISSING_DOCUMENT_MESSAGE = 'Document not available: file:///c%3A/Users/x/My%20Project/src/Foo.jl.'
...
suite('isExpectedDocumentStateError', () => {
    ...
    test('recognises the InvalidParams answer for a missing document by its message', () => {
        assert.strictEqual(
            isExpectedDocumentStateError(new ResponseError(ErrorCodes.InvalidParams, FILE_MISSING_DOCUMENT_MESSAGE)),
            true
        )
        assert.strictEqual(
            isExpectedDocumentStateError(new ResponseError(ErrorCodes.InvalidParams, MISSING_DOCUMENT_MESSAGE)),
            true
        )
    })

    test('does not match any other InvalidParams answer', () => {
        assert.strictEqual(
            isExpectedDocumentStateError(new ResponseError(ErrorCodes.InvalidParams, 'Invalid params')),
            false
        )
        assert.strictEqual(
            isExpectedDocumentStateError(
                new ResponseError(ErrorCodes.InvalidParams, `${FILE_MISSING_DOCUMENT_MESSAGE} Retrying.`)
            ),
            false
        )
    })

    test('classifies a response error by its code, not by a message it happens to carry', () => {
        assert.strictEqual(
            isExpectedDocumentStateError(new ResponseError(ErrorCodes.InternalError, NO_DOCUMENT_MESSAGE)),
            false
        )
        assert.strictEqual(
            isExpectedDocumentStateError(new ResponseError(ErrorCodes.InternalError, FILE_MISSING_DOCUMENT_MESSAGE)),
            false
        )
    })
})

// A notebook cell URI with a space in its path, as it reached telemetry from
// 1.246.1 after VS Code's cleaning: the `%20` became a space and the path
// pieces were redacted, so the URI is no longer a run of non-whitespace.
const CLEANED_NOTEBOOK_CELL_MESSAGE =
    'Document not available: vscode-notebook-cel<REDACTED: user-file-path> e <REDACTED: user-file-path>#W2sZmlsZQ==.'

/**
 * What VS Code's telemetry logger hands `sendErrorData` for `err`: a new
 * plain `Error` carrying the cleaned message, with `name` copied across. The
 * prototype and the code are not.
 */
function rebuiltByVsCode(err: Error, cleanedMessage: string): Error {
    const rebuilt = new Error(cleanedMessage)
    rebuilt.name = err.name
    return rebuilt
}

suite('tagDocumentStateError', () => {
    test('names the document-state answers, and changes nothing else about them', () => {
        for (const [code, message] of [
            [-33100, NO_DOCUMENT_MESSAGE],
            [-33101, VERSION_MISMATCH_MESSAGE],
            [ErrorCodes.InvalidParams, FILE_MISSING_DOCUMENT_MESSAGE],
        ] as const) {
            const err = new ResponseError(code, message, { detail: 1 })
            assert.strictEqual(tagDocumentStateError(err), err, `code ${code}`)
            assert.strictEqual(err.name, DOCUMENT_STATE_ERROR_NAME, `code ${code}`)
            assert.ok(err instanceof ResponseError, `code ${code}`)
            assert.strictEqual(Object.getPrototypeOf(err), ResponseError.prototype, `code ${code}`)
            assert.strictEqual(err.code, code)
            assert.strictEqual(err.message, message)
            assert.deepStrictEqual(err.data, { detail: 1 })
            assert.strictEqual(isExpectedDocumentStateError(err), true, `code ${code}`)
        }
    })
    ...
    test('leaves every other error alone', () => {
        const others: unknown[] = [
            ...
            new ResponseError(ErrorCodes.InternalError, NO_DOCUMENT_MESSAGE),
            new ResponseError(
                ErrorCodes.PendingResponseRejected,
                'Pending response rejected since connection got disposed'
            ),
            // Not a response error, so its code is not there to vouch for it.
            new Error(FILE_MISSING_DOCUMENT_MESSAGE),
            new TypeError('boom'),
        ]
        for (const err of others) {
            const name = (err as Error).name
            assert.strictEqual(tagDocumentStateError(err), err)
            assert.strictEqual((err as Error).name, name, String(err))
        }
        assert.strictEqual(tagDocumentStateError('nope'), 'nope')
        assert.strictEqual(tagDocumentStateError(undefined), undefined)
    })

    test('a named answer is still recognised after VS Code rebuilds it with a cleaned message', () => {
        // A single-line message containing a URL is replaced whole.
        const fromFile = tagDocumentStateError(
            new ResponseError(ErrorCodes.InvalidParams, FILE_MISSING_DOCUMENT_MESSAGE)
        )
        const noDocument = tagDocumentStateError(new ResponseError(-33100, NO_DOCUMENT_MESSAGE))
        const fromNotebook = tagDocumentStateError(
            new ResponseError(
                ErrorCodes.InvalidParams,
                'Document not available: vscode-notebook-cell:/c%3A/Users/x/My%20Notebooks/a.ipynb#W2sZmlsZQ==.'
            )
        )
        for (const rebuilt of [
            rebuiltByVsCode(fromFile, '<REDACTED: URL>'),
            rebuiltByVsCode(noDocument, '<REDACTED: URL>'),
            rebuiltByVsCode(fromNotebook, CLEANED_NOTEBOOK_CELL_MESSAGE),
        ]) {
            assert.strictEqual(rebuilt instanceof ResponseError, false)
            assert.strictEqual(isExpectedDocumentStateError(rebuilt), true, rebuilt.message)
            assert.strictEqual(isLanguageServerResponseNoise(rebuilt), true, rebuilt.message)
        }
    })

    test('without the name, a rebuilt report is not recognised and is still reported', () => {
        // Nothing in these says what they were: the message patterns cannot
        // safely stretch to them, which is why the name has to carry it.
        for (const message of ['<REDACTED: URL>', CLEANED_NOTEBOOK_CELL_MESSAGE]) {
            assert.strictEqual(isExpectedDocumentStateError(new Error(message)), false, message)
            assert.strictEqual(isLanguageServerResponseNoise(new Error(message)), false, message)
        }
        // A generic InvalidParams answer is never named, so it is reported too.
        const invalidParams = tagDocumentStateError(new ResponseError(ErrorCodes.InvalidParams, 'Invalid params'))
        const rebuilt = rebuiltByVsCode(invalidParams, 'Invalid params')
        assert.strictEqual(isExpectedDocumentStateError(rebuilt), false)
        assert.strictEqual(isLanguageServerResponseNoise(rebuilt), false)
    })

    test('the name is a value VS Code cleaning leaves untouched', () => {
        // The URL, file path and generic secret heuristics of VS Code's
        // `cleanData`, which would otherwise rewrite it.
        assert.strictEqual(/[a-zA-Z][a-zA-Z0-9+.-]*:\/\/[^\s]*/.test(DOCUMENT_STATE_ERROR_NAME), false)
        assert.strictEqual(/[\\/]/.test(DOCUMENT_STATE_ERROR_NAME), false)
        assert.strictEqual(
            /(key|token|sig|secret|signature|password|passwd|pwd|android:value)[^a-zA-Z0-9]/i.test(
                DOCUMENT_STATE_ERROR_NAME
            ),
            false
        )
        assert.match(DOCUMENT_STATE_ERROR_NAME, /^[A-Za-z]+$/)
    })
})
