...
## [Unreleased]
### Added
- A Julia icon in the status bar shows what the language server is doing. The icon spins with a compact `done/total` counter while the server starts up or its indexing child processes are still working, and hovering it shows a flyout with the details: one line per environment being indexed (with progress), queued and failed environments (with the failure reason), how many indexer processes are alive, and the active Julia environment, plus quick links to restart the server or open its log. The per-process details need a language server with the `julia/publishServerStatus` notification; with an older server the item still reflects the starting/running/crashed state. This replaces the previous transient "Starting Language Server" and "Language Server Crashed" status bar messages.
...
- A language server that cannot create its symbol store directory now reports what shape the storage path it was given had, rather than only the failure itself, and the client records that path in the language server output channel. The crash this diagnoses — `mkdir` refused on a path that lost a separator somewhere between the client and the server — has no explanation yet, and the reports so far carry nothing that would identify where the value came from. No path is ever transmitted, only derived facts about its shape ([#4216](https://github.com/julia-vscode/julia-vscode/pull/4216)).
- A language server request that fails for a reason the extension does not already recognise now records which request it was and what the server answered, in the language server output channel and in telemetry. Neither survives to a crash report otherwise — VS Code rebuilds such a rejection as a plain error, leaving only its message — which is why reports whose message is nothing but a URL currently cannot be traced to the request that produced them. No message text is recorded, only the method name and the numeric error code ([#4233](https://github.com/julia-vscode/julia-vscode/pull/4233)).

### Fixed
- Starting a debug session on a file that is not valid Julia now says so in the debug console instead of filing a crash report and leaving the user with no explanation. The debug session ends the way a finished one does, and the REPL it ran in keeps its debugger ([DebugAdapter.jl#135](https://github.com/julia-vscode/DebugAdapter.jl/pull/135)).
- A file or folder appearing in a workspace at a path the language server is not allowed to look at — a OneDrive placeholder the client has not materialized, a directory whose permissions exclude the server, a network mount that went away — no longer takes the server down. Asking whether such a path is a directory raises rather than answering, and the watched-files handler did not expect that; a user whose project sat in a OneDrive folder lost the server repeatedly while only editing files. Such a path is now treated as one that cannot be scanned, which is what the handler already does with any other unreadable path ([LanguageServer.jl#1480](https://github.com/julia-vscode/LanguageServer.jl/pull/1480)).
- The language server is told how many times the client has automatically restarted it after a crash, so its crash reports for a document sync error can say whether a restart preceded it ([#4205](https://github.com/julia-vscode/julia-vscode/pull/4205)).

### Fixed
- Packages are indexed again when the language server's indexing process runs on Julia 1.8 or older. The symbol indexing code, which that process shares with the language server, had started enumerating module bindings through a function that only exists from Julia 1.9 and with keyword syntax that does not parse before 1.5, so every indexing run on an older Julia failed and those packages got no symbol information. Checking the older versions turned up more of the same — functions newer than the oldest supported Julia, a 1.0 manifest format the indexer did not understand, and on 1.0 and 1.1 a package's own `Base.show` and similar overloads going missing from its cache — which are fixed too, and a CI job now indexes Base on Julia 1.0, 1.1, 1.3, 1.6 and 1.8 ([julia-vscode/JuliaWorkspaces.jl#348](https://github.com/julia-vscode/JuliaWorkspaces.jl/pull/348)).
- Pressing Step Into just as the debugged code finishes no longer produces a crash report. The debugger confirms a step before it takes it, and only tells VS Code that the code has run to its end once the step is done. VS Code sends Step Into whatever state the debugger is in, and asks for the variables of the place it last stopped at, so both kinds of request could reach the debugger after the code being debugged was gone. The debugger now answers them with an error instead of failing ([julia-vscode/DebugAdapter.jl#136](https://github.com/julia-vscode/DebugAdapter.jl/pull/136)).
- Debugging code that fails to load partway through, most often because it uses a macro it never imported (`L"..."` without `using LaTeXStrings`), no longer produces a crash report. The debugger expands macros one top-level expression at a time as it reaches each one, so such a failure arrived after the file had parsed fine, and nothing caught it: the user got a crash report instead of the error `include` would have shown them. The error now appears in the debug console, with the same message and location, and the run ends as if the code had finished ([julia-vscode/DebugAdapter.jl#137](https://github.com/julia-vscode/DebugAdapter.jl/pull/137)).
- Two modules that import the same name from each other (`using ..B: f` in one and `using ..A: f` in the other, easily written by mistake or mid-edit) no longer crash the language server when you hover the name, ask for completions or signature help near it, or go to its definition. The two imports ended up pointing at each other, and everything that follows an imported name to its definition went round that loop until Julia ran out of stack. Neither module actually has the name, so the imports are now left unresolved, which is what they are, and those features stop if they ever meet such a loop anyway ([julia-vscode/JuliaWorkspaces.jl#349](https://github.com/julia-vscode/JuliaWorkspaces.jl/pull/349)).
- A language server that is short of memory no longer crashes while loading a package's cached symbol information. The caches that failed were small — a few megabytes for SCIP, Distributions and JuMP — so the process was already at its memory ceiling, and Julia does not run the garbage collector before giving up on an allocation. The server now collects and tries once more, and if the cache still does not fit it carries on without that package's symbols until it is restarted, with a warning in the language server output, instead of failing or trying to rebuild a cache that is perfectly fine ([julia-vscode/JuliaWorkspaces.jl#347](https://github.com/julia-vscode/JuliaWorkspaces.jl/pull/347)).
- Typing `$(` into a string that already interpolates a bare variable — `"I$x$()"`, which is what the editor's auto-closed parenthesis leaves behind — no longer crashes the language server's analysis of the file. The parser records an empty `$()` as a placeholder value, and the code that walks a string's pieces mistook that placeholder for a piece of text, lost its place and read past the end; in strings where it did not crash, it handed out the pieces out of order. A triple-quoted string containing nothing but an escaped newline, which failed the same way, is fixed too ([julia-vscode/CSTParser.jl#409](https://github.com/julia-vscode/CSTParser.jl/pull/409)).
- A language server answer saying it does not have a document is no longer reported as a crash when the document's URI contains a space or is a `file://` URI. Crash reporting receives such an error from VS Code, which does not pass on the error itself but a rebuilt copy whose message it has cleaned of anything that might identify the user: a `%20` becomes a space, path-like runs are redacted, and a message containing a `file://` URL is replaced by `<REDACTED: URL>` in its entirety. Every one of these answers names the document, so for such a document nothing recognisable was left of it, and notebook cells and files under a folder with a space in its name kept being reported. The language client now marks such an answer by its error name while the server's error code is still attached, and the name is the one part VS Code passes on unchanged. This is also what most of the reports whose message is nothing but a URL, which [#4233](https://github.com/julia-vscode/julia-vscode/pull/4233) set out to trace, turn out to be: not a message that was only a URL, but one VS Code blanked because it contained one ([#4240](https://github.com/julia-vscode/julia-vscode/pull/4240)).
- A debug session no longer fails outright when the socket the extension picked for it cannot be created by the Julia process. The two processes are not always given the same temp directory — macOS is the case in telemetry, where every debug session one user started died on `could not listen on path` — and a socket path also has a length limit that a deep temp directory can exceed. The debugger now falls back to a socket name of its own and reports it over the handshake that already exists for saying it is ready, and the extension attaches to the name it is told rather than the one it proposed. A debugger that cannot create a socket of its own choosing either is still reported ([#PR](https://github.com/julia-vscode/julia-vscode/pull/PR)).
- The "Show modules" and "Hide modules" buttons of the Julia Workspace view now work when no REPL is running, instead of failing with a crash report. The buttons are shown whether or not a REPL is connected — with no session at all, or with only notebook sessions — but the command assumed there was one to apply the change to. It now applies to whichever sessions exist and saves the setting either way, so a REPL started afterwards shows or hides modules as chosen ([#4239](https://github.com/julia-vscode/julia-vscode/pull/4239)).
- A debug session no longer fails outright when the socket the extension picked for it cannot be created by the Julia process. The two processes are not always given the same temp directory — macOS is the case in telemetry, where every debug session one user started died on `could not listen on path` — and a socket path also has a length limit that a deep temp directory can exceed. The debugger now falls back to a socket name of its own and reports it over the handshake that already exists for saying it is ready, and the extension attaches to the name it is told rather than the one it proposed. A debugger that cannot create a socket of its own choosing either is still reported ([#4234](https://github.com/julia-vscode/julia-vscode/pull/4234)).
- A language server or test item controller that is terminated with `SIGTERM` by something other than this extension — closing a remote window, a session or container being torn down, a person running `kill` — is no longer reported as an out-of-memory kill, and no longer pops up an error saying it ran out of memory. That report exists to catch the language server growing until the kernel kills it, and an out-of-memory kill is always a `SIGKILL`; the reports actually arriving were `SIGTERM`s from remote sessions on a machine with 438 GiB of memory free. Such a stop is still written to the output channel and still counted in telemetry, and a `SIGKILL` is reported exactly as before ([#4231](https://github.com/julia-vscode/julia-vscode/pull/4231)).
- Tagging a new package version now checks the GitHub credentials before it starts Julia, and re-runs the sign in flow if they have gone stale. VS Code hands out whatever GitHub session it has cached without revalidating it, so a token that was revoked server-side — the VS Code OAuth app's authorization removed, a password reset, an organization's SAML single sign on lapsed — was passed straight to PkgDev, which died with a bare `401 Bad credentials` Julia stacktrace in a terminal the extension no longer controls. If the credentials are still rejected after signing in again, the command now says so and explains what to do instead of tagging nothing. Dismissing the GitHub sign in prompt also no longer files a crash report ([#4230](https://github.com/julia-vscode/julia-vscode/pull/4230)).
- A package cache file in the depot that cannot be opened — blocked by a Windows application control policy or an antivirus, corrupt, or on a filesystem that will not map it — now shows the same actionable message as any other broken depot instead of being reported as a language server crash. The launchers' advice also no longer talks only about write permissions, since that is not what is wrong when something is blocking the files ([#4227](https://github.com/julia-vscode/julia-vscode/pull/4227)).
- A Julia that starts but cannot run the extension's depot queries — a version manager that built the channel path around a depot path that still had its separator in it, a depot that needs `Pkg.instantiate()`, a stale precompiled image — no longer takes down the environment switcher with a crash report. The switcher now opens without the environments stored in the depots, says why, and points at the Julia Executables output channel for the reason ([#4228](https://github.com/julia-vscode/julia-vscode/pull/4228)).
- A language server answer that says it does not have a document, or does not have it at the version a request was built against, no longer produces a crash report. The server deliberately does not track every buffer VS Code hands a command — a `git:` diff view is the common one — and an edit reaches it after the keystroke that triggered the request, so it reports both as ordinary protocol errors. Nothing was catching them, so pressing Ctrl+Enter in such a buffer filed a report and did nothing else. The block-range and module-at lookups now fall back to the answer they already use when there is nothing to report ([#4225](https://github.com/julia-vscode/julia-vscode/pull/4225)).
- A test run no longer crashes outright when one of its test items changes while the run is being prepared. A run is assembled in two steps with a language server round-trip per file in between, and a republish of a file's test items in that window replaces every test item for it, leaving the run holding items whose details it can no longer look up — the first field read off one of those took the whole run down. Such an item is now skipped, with a line in the test run output saying so, and the rest of the run goes ahead ([#4226](https://github.com/julia-vscode/julia-vscode/pull/4226)).
- A language feature request for a position the document does not have is no longer sent to the language server, where it crashed the request. Nothing between the caller and the server validates a provider position — VS Code hands it to the provider untouched and vscode-languageclient converts it verbatim — so anyone asking a provider about, say, `new Position(document.lineCount, 0)`, which is one line past the end, reached the server and took the request down with it. Such requests are now dropped in the client, which reports no result for them, and counted in telemetry. This affects only requests that would have failed anyway; nothing changes about the messages that are sent. An out-of-range position that the client could *not* see was out of range still fails loudly on the server, because that case means the two sides disagree about the document and is worth fixing rather than hiding.
- A language server precompile failure now shows its actionable "check depot permissions" message instead of crashing with a `KeyError`: the message read `JULIA_DEPOT_PATH` from the environment, which the extension does not set. A permission error on the compiled cache is also recognised as a precompile failure rather than reported as a crash ([#4205](https://github.com/julia-vscode/julia-vscode/pull/4205)).
- A broken user environment (e.g. a stale precompiled image) no longer produces a crash report and a dead language server when the default environment is resolved: the failed Julia invocation now degrades to a best-effort default environment path ([#4205](https://github.com/julia-vscode/julia-vscode/pull/4205)).
- A test run no longer hangs forever when the test item controller cannot be started at all. A failed launch (a missing or unusable Julia) leaves the process object in place without ever reporting an exit, so the extension took it for a live controller and sent the run into a pipe nobody was reading. The launch outcome is now awaited, and a failure ends the run with an error instead.
- Failures inside the extension's handling of messages from the test item controller are no longer discarded in silence. Every such handler ran without any error reporting attached, so a fault in one — updating an item's result, tracking a test process, opening its output channel — produced no message, no log line and no crash report; the symptom was a test item that simply stopped updating. Debugging a test item was affected the same way: if the debug session failed to attach, nothing said so and the test process waited for a debugger that was never coming.
- The Julia Test Item Controller output channel now records the controller's exit and any connection-level failure, so a controller that dies mid-run leaves an explanation behind. A controller that could not be started, or that exited unexpectedly, is also reported through crash telemetry when it did not get far enough to report itself.
- The advice to install the Juliaup `release` channel is no longer shown for unrelated failures: the lookup swallowed every error, so any other problem was reported as a missing channel and following the advice did not help.
...
- Internal errors in a range of extension code paths are no longer silently swallowed and now reach crash telemetry. This covers the task provider (which returned an empty task list on any failure), interactions with the REPL process (workspace variable views, lazy tables, plot saving, code-cell debugging, environment activation and the configuration-change notifications), and the juliaup channel probing, which treated every error as "channel not installed". Expected conditions — a missing Julia installation, filesystem problems, a REPL or language server that went away — are still handled quietly. The REPL-side server reports failures of its own internals (REPL integration installation, module resolution, internal eval errors) the same way ([#4202](https://github.com/julia-vscode/julia-vscode/pull/4202)).
- Clicking "Install" on the juliaup recommendation notification now runs the full installation flow, including the language server restart that makes the new juliaup take effect. A failed or declined installation no longer produces a crash report, no longer leaves the extension with a broken juliaup handle that caused later errors, and declining is no longer reported as a failure ([#4208](https://github.com/julia-vscode/julia-vscode/pull/4208)).
- Formatting a file that cannot be parsed as Julia code, and language server request failures that the server already reports itself, no longer additionally produce extension-side crash reports ([#4209](https://github.com/julia-vscode/julia-vscode/pull/4209)).
- Clicking the task runner's status bar item after its terminal had been closed no longer crashes, and the status bar item is disposed together with the runner ([#4207](https://github.com/julia-vscode/julia-vscode/pull/4207)).
- A language server or test item controller killed by Windows at session teardown (logoff or shutdown) no longer produces crash reports: the kill itself and the restart attempts that cannot initialise in the ending session carry no crash information. Unexplained external terminations and signals, including the macOS out-of-memory kill, are still reported ([#4214](https://github.com/julia-vscode/julia-vscode/pull/4214)).
- An out-of-memory kill of the language server, and any other termination forced on it from outside the process tree such as a container stop or a `kill -9`, now says so: an error notification with a button to open the logs, an entry in the language server output channel, and a report carrying the free and total memory, any cgroup limit, the Julia it was running and the tail of its stderr. The language server is this extension's long-lived process, so memory growth that ends in the kernel stepping in can be a defect here rather than a short machine, and that is only visible if the figures are reported. This supersedes the entry below, which stopped reporting such a kill at all. Terminations the extension itself causes, which end in a `SIGTERM` from our own shutdown path, are still not reported.
- An out-of-memory kill of the language server, and any other termination forced on it from outside the process tree such as a container stop or a `kill -9`, no longer produces a crash report. Neither the extension nor Julia sends those signals, so they describe the machine rather than a fault, and the report carried no stack and nothing to act on; this narrows the entry above, which still reported them. Such a kill is instead counted as a usage event alongside the machine's memory figures, so that language server memory pressure stays visible. Native crash signals such as `SIGSEGV` are still reported as crashes ([#4217](https://github.com/julia-vscode/julia-vscode/pull/4217)).
- A test item controller that cannot precompile, e.g. on a depot that needs `Pkg.instantiate()`, now shows an actionable error notification with FAQ and log buttons instead of filing a crash report, mirroring the language server's existing behaviour. A language server whose Julia installation cannot spawn its own precompilation workers (e.g. an environment manager replaced the binary mid-session) is likewise treated as a broken installation rather than a server crash ([#4213](https://github.com/julia-vscode/julia-vscode/pull/4213)).
- A test item controller or language server that dies with a write in flight no longer produces a duplicate crash report for the stranded write: on Windows such a write fails with `write EOF`, the counterpart of the already-classified `write EPIPE`, and the process death itself is reported separately. The controller's graceful-shutdown notification also no longer leaves an unhandled rejection when its write fails ([#4212](https://github.com/julia-vscode/julia-vscode/pull/4212)).
- A test failure without expected/actual output or a source location no longer crashes the extension while the result is rendered: the test item controller omits those optional fields from its wire messages, and the handler's null-only guard let the resulting `undefined` through to `stripAnsi` ([#4211](https://github.com/julia-vscode/julia-vscode/pull/4211)).
- Interrupting a REPL that is no longer there no longer produces a crash report. Pressing Ctrl+C three times within a second escalates to a `SIGINT` on the terminal process, which crashed when the terminal had been closed in the meantime, and reported a failure to signal an already-exited process as a crash because the guard against that sat in a `try` the signalling never ran in.
- Stopping the test item controller no longer produces a crash report, and a controller killed from outside now says so. Both the “kill” action and the fallback of the graceful shutdown send a plain `SIGTERM`, so every ordinary teardown — closing VS Code, stopping the controller from the process view — was reported as a crash that carried no crash. The controller now records that it is being stopped on purpose, so a kill it did not ask for is told apart from its own. Such a kill — most often the operating system running out of memory — is now shown as an error notification with a button to open the logs, written to the Julia Test Item Controller output channel, and reported with the free and total memory, any cgroup limit, and how many test processes were alive, since a run that grows until the kernel intervenes can be a leak on our side rather than the machine being short. Native crash signals such as `SIGSEGV` are still reported as before.
...
- Out-of-bounds diagnostic ranges are now clamped instead of crashing the request ([LanguageServer.jl#1401](https://github.com/julia-vscode/LanguageServer.jl/pull/1401))
...
- Improved the juliaup installation UX ([#4004](https://github.com/julia-vscode/julia-vscode/pull/4004), [#3997](https://github.com/julia-vscode/julia-vscode/pull/3997))
