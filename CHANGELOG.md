...
### Fixed
- The language server no longer runs out of memory reading a package's cached symbol information. Each cache file was read into memory in one piece before being parsed, and the one for `LinearAlgebra` alone is about 50 MB, so a server that was already using a lot of memory — or a 32-bit Julia — could not get that single block and failed. The file is now read in small chunks, just as fast, and a file too large to be a real cache is treated as corrupt and rebuilt instead ([julia-vscode/JuliaWorkspaces.jl#347](https://github.com/julia-vscode/JuliaWorkspaces.jl/pull/347)).
