...
- A language server crash no longer cascades into a crash loop through stale document notifications. When the client auto-restarts a crashed server, vscode-languageclient parks `didOpen`/`didChange`/`didClose`/`didSave` notifications fired during the down window and delivers them to the new server after its document replay, where the out-of-date versions tripped the server's sync assertions. Such stale notifications are now dropped when the replay has already superseded them, and the server is told how many times it has been restarted so its crash reports can say so ([#4205](https://github.com/julia-vscode/julia-vscode/pull/4205)).
