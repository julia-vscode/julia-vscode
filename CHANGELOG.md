...
- Stopping the test item controller no longer produces a crash report. Both the “kill” action and the fallback of the graceful shutdown send a plain `SIGTERM`, so every ordinary teardown — closing VS Code, stopping the controller from the process view — was reported as a crash that carried no crash. The controller now records that it is being stopped on purpose, the way the language server does, so a kill it did not ask for is still told apart from its own. Such an outside kill, an out-of-memory kill in particular, is counted as a usage event alongside the machine's memory figures rather than reported as a crash, mirroring the language server; native crash signals such as `SIGSEGV` are still reported.
