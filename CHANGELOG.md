...
- Stopping the test item controller no longer produces a crash report. Both the "kill" action and the fallback of the graceful shutdown send a plain `SIGTERM`, so every ordinary teardown — closing VS Code, stopping the controller from the process view — was reported as a crash that carried no crash. Kill signals from outside, such as an out-of-memory kill, are excluded for the same reason they are for the language server; native crash signals such as `SIGSEGV` are still reported.
