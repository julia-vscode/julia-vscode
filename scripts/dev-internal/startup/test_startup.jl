...
#
# Usage:
#   julia scripts/terminalserver/test_startup.jl [--profile] [--snoop] [--snoop-invalidations] [--verbose]
#
...
#   7. Optionally runs SnoopCompile @snoop_invalidations on using VSCodeServer to observe method invalidations
#
# Options:
...
#   --snoop-invalidations   Run SnoopCompile @snoop_invalidations on using VSCodeServer to report invalidation trees
...
const DO_SNOOP_INVALIDATIONS = "--snoop-invalidations" in ARGS
...
DO_SNOOP_INVALIDATIONS && push!(worker_args, "--snoop-invalidations")
VERBOSE && push!(worker_args, "--verbose")

worker_script = joinpath(SCRIPT_DIR, "test_startup_worker.jl")
worker_project_args = (DO_SNOOP || DO_SNOOP_INVALIDATIONS) ? `--project=$(SCRIPT_DIR)` : ``
...
if DO_SNOOP_INVALIDATIONS
    inv_report = joinpath(results_dir, "invalidations_report.txt")
    if isfile(inv_report)
        println()
        print(read(inv_report, String))
        println()
    else
        @warn "SnoopCompile invalidations report was not generated by worker"
    end
end
