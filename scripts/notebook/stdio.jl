struct JuliaNotebookStdio{IO_t <: IO} <: Base.AbstractPipe
    io::IOContext{IO_t}
end
...
Base.unlock(io::JuliaNotebookStdio) = unlock(io.io.io)
Base.in(key_value::Pair, io::JuliaNotebookStdio) = in(key_value, io.io)
Base.haskey(io::JuliaNotebookStdio, key) = haskey(io.io, key)
Base.getindex(io::JuliaNotebookStdio, key) = getindex(io.io, key)
Base.get(io::JuliaNotebookStdio, key, default) = get(io.io, key, default)
Base.displaysize(io::JuliaNotebookStdio) = displaysize(io.io)
Base.unwrapcontext(io::JuliaNotebookStdio) = Base.unwrapcontext(io.io)
Base.setup_stdio(io::JuliaNotebookStdio, readable::Bool) = Base.setup_stdio(io.io.io, readable)

function Base.redirect_stdout(io::JuliaNotebookStdio)
    io[:jupyter_stream] != "stdout" && throw(ArgumentError(string("expecting ", "stdout", " stream")))
    Core.eval(Base, Expr(:(=), Symbol("stdout"), io))
    return io
end

function Base.redirect_stderr(io::JuliaNotebookStdio)
    io[:jupyter_stream] != "stderr" && throw(ArgumentError(string("expecting ", "stderr", " stream")))
    Core.eval(Base, Expr(:(=), Symbol("stderr"), io))
    return io
end

function Base.redirect_stdin(io::JuliaNotebookStdio)
    io[:jupyter_stream] != "stdin" && throw(ArgumentError(string("expecting ", "stdin", " stream")))
    Core.eval(Base, Expr(:(=), Symbol("stdin"), io))
    return io
end
            ...
            if nb > 0
                ...
                if stdio_bytes[] >= max_output_per_request[]
                    ...
                    if stdio_bytes[] - nb < max_output_per_request[]
                        send_msg_to_vscode(conn, "stderr", string(current_request_id[], ";", "Excessive output truncated after $(stdio_bytes[]) bytes."))
                    end
                else
...
function send_stream(name::AbstractString)
    ...
    if buf.size > 0
        ...
        if isvalid(String, s)
            ...
            b64 = Base64.Base64EncodePipe(sbuf)
            write(b64, d)
            write(b64, dextra)
            close(b64)
            print(sbuf, '\n')
        ...
        send_msg_to_vscode(conn, name, string(current_request_id[], ";", s))
    end
end
...
function Base.getpass(input::JuliaNotebookStdio, output::JuliaNotebookStdio, message::AbstractString)
    check_prompt_streams(input, output)
